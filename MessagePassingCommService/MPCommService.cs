...
namespace MessagePassingComm
{
  ...
  public class Receiver : IMessagePassingComm
  {
    ...
    public bool openFileForWrite(string name)
    {
      try
      {
        string writePath = Path.Combine(TestharnessMockEnvironment.fileStorage, name);
    ...
    public bool openFileForWriteAddress(string name, string address)
    {
      try
      {
        string portnumber = address.Substring(17,4);
        string newpath = ChildBuilderEnvironment.fileStorage + "/" + "ChildBuilder#"+ portnumber;
        string writePath = Path.Combine(newpath, name);
        fs = File.OpenWrite(writePath);
        return true;
      }
      catch(Exception ex)
  ...
  public class Sender
  {
    ...
    /*----< uploads file to Receiver instance with appropriate address >--------------------*/
    public bool postFile(string fileName , string address)
    {
      FileStream fs = null;
      long bytesRemaining;
      ...
      {
        string path = "";
        string str = fileName.Substring(fileName.Length - 2, 2);
        if(str.Equals("cs"))
                path = Path.Combine(RepoMockEnvironment.codeStorage, fileName);
        else
                path = Path.Combine(RepoMockEnvironment.testRequestsStorage, fileName);
        fs = File.OpenRead(path);
        bytesRemaining = fs.Length;
        channel.openFileForWriteAddress(fileName, address);
        while (true)
        {
          long bytesToRead = Math.Min(RepoMockEnvironment.blockSize, bytesRemaining);
          byte[] blk = new byte[bytesToRead];
          long numBytesRead = fs.Read(blk, 0, (int)bytesToRead);
          bytesRemaining -= numBytesRead;
      ...
      return true;
        ...
        /*----< uploads file to Receiver instance with appropriate address >--------------------*/
    public bool postFile(string fileName , string childlibrariesStoragepath, bool flag)
    {
      FileStream fs = null;
      long bytesRemaining;
      ...
      try
      {
        string path = Path.Combine(childlibrariesStoragepath, fileName);
        fs = File.OpenRead(path);
        bytesRemaining = fs.Length;
        channel.openFileForWrite(fileName);
        while (true)
...
public class Comm
  {
    ...
    public bool postFile(string filename,string address)
    {
      return sndr.postFile(filename, address);
    }
    /*----< called by remote Comm to upload file >-----------------*/
    public bool postFile(string filename,string address , bool flag)
    {
      return sndr.postFile(filename, address, flag);
    }
  }
  ///////////////////////////////////////////////////////////////////
  ...
  class TestPCommService
  {
    ...
    public static bool compareMsgs(CommMessage msg1, CommMessage msg2)
    {
      ...
      if (msg1.testFiles.Count != msg2.testFiles.Count)
        return false;
      for(int i=0; i<msg1.testFiles.Count; ++i)
      {
        if (msg1.testFiles[i] != msg2.testFiles[i])
          return false;
      }
